...
const mockLeapYearLoader: () => Promise<any[]> = () =>
  new Promise((resolve) => {
    resolve(new Array(HOURS_PER_LEAP_YEAR).fill(1));
  });

describe("Working Data calculations", () => {
  let spy: jest.SpyInstance<
    JSX.Element[],
    [charts: { title: string; items: { [key: string]: number } }[]]
  >;
  beforeAll(() => {
  ...
  describe("Capital Cost Breakdown", () => {
    it("calculates electrolyser CAPEX as expected", async () => {
      ...
      await waitFor(() => {
        const result = spy.mock.calls[0][0].filter(
    ...
    it("calculates capital cost breakdown for grid connected methane with wind and battery", async () => {
      render(
        <WorkingData
          data={gridConnectedMethaneWindWithBatteryAndDegradationScenario.data}
          inputConfiguration="Advanced"
          loadSolar={mockLeapYearLoader}
          loadWind={mockLeapYearLoader}
          location={
  ...
  describe("Indirect Cost Breakdown", () => {
    it("calculates electrolyser indirect costs", async () => {
      ...
      await waitFor(() => {
        const result = spy.mock.calls[0][0].filter(
          (value: any) => value.title === "Indirect Cost Breakdown"
        );
        expect(result).toHaveLength(1);
  ...
  it("calculates indirect cost breakdown for wind methane", async () => {
    render(
      <WorkingData
        data={gridConnectedMethaneWindWithBatteryAndDegradationScenario.data}
        inputConfiguration="Advanced"
    ...
    await waitFor(() => {
      ...
      expect(result).toHaveLength(1);
      const chartData = result[0].items;
      expect(chartData["Methane EPC"]).toEqual(33_479_400);
      expect(chartData["Methane Land"]).toEqual(8_369_850);
      expect(chartData["CC EPC"]).toEqual(371_996_800);
